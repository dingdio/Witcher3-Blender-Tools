...
class SMipData():
    ...
    def Read(self, f, size):
        ...
        if self.__CR2WFILE.CR2WExport[self.__CR2WFILE.currentExport].objectFlags == 0: #is uncooked export
            self.Mip = CByteArray().Read(f, size)

class CCompressedBufferTexture:
    def __init__(self, CR2WFILE, Name = "SMipData"):
        self.__CR2WFILE = CR2WFILE
        self.Name = Name
        self.bufferData = []
    def Read(self, f, size, count, buffer_type = SMipData):
        m_count = count
        for _ in range(0, m_count):
            data = buffer_type(self.__CR2WFILE)
            data.Read(f,0)
