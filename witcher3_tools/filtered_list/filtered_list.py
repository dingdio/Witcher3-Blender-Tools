...
class CModUiFilteredList(ABC):
    ...
    def __init__(self):
        ...
        self._expandAll: bool = False
        self._autoCollapseCategories: bool = True
        self._openedCategories = set()
        self._deferAutoCollapseUntilNextCategory: bool = False
    ...
    # ------------------------------------------------------------------------
    def setExpandAll(self, expand: bool):
        self._expandAll = bool(expand)
    # ------------------------------------------------------------------------
    def getExpandAll(self) -> bool:
        return bool(self._expandAll)
    # ------------------------------------------------------------------------
    def setAutoCollapseCategories(self, enabled: bool):
        enabled = bool(enabled)
        if self._autoCollapseCategories == enabled:
            return
        self._autoCollapseCategories = enabled
        self._expandAll = False

        if enabled:
            # Preserve currently opened categories until the next category click.
            if not self._openedCategories:
                if self._selectedCat1 != "":
                    self._openedCategories.add("CAT1" + self._selectedCat1)
                if self._selectedCat2 != "":
                    self._openedCategories.add("CAT2" + self._selectedCat2)
                if self._selectedCat3 != "":
                    self._openedCategories.add("CAT3" + self._selectedCat3)
            self._deferAutoCollapseUntilNextCategory = True
            return

        self._deferAutoCollapseUntilNextCategory = False
        self._openedCategories.clear()
        if not enabled:
            if self._selectedCat1 != "":
                self._openedCategories.add("CAT1" + self._selectedCat1)
            if self._selectedCat2 != "":
            ...
            if self._selectedCat3 != "":
                self._openedCategories.add("CAT3" + self._selectedCat3)
    # ------------------------------------------------------------------------
    def getAutoCollapseCategories(self) -> bool:
        return bool(self._autoCollapseCategories)
    # ------------------------------------------------------------------------
    def clearOpenedCategories(self):
        self._openedCategories.clear()
    # ------------------------------------------------------------------------
    def toggleCategory(self, id: str):
        if not id or not id.startswith("CAT"):
            return
        ...
        if self._autoCollapseCategories:
            self._deferAutoCollapseUntilNextCategory = False
            self._openedCategories.clear()
            if prefix == "CAT1":
                if self._selectedCat1 == catId:
                    self._selectedCat1 = ""
                    self._selectedCat2 = ""
                    self._selectedCat3 = ""
                else:
                    self.setSelection(id, True)
            elif prefix == "CAT2":
                if self._selectedCat2 == catId:
                    self._selectedCat2 = ""
                    self._selectedCat3 = ""
                else:
                    self.setSelection(id, True)
            elif prefix == "CAT3":
                if self._selectedCat3 == catId:
                    self._selectedCat3 = ""
                else:
                    self.setSelection(id, True)
            return

        if id in self._openedCategories:
            self._openedCategories.remove(id)
            if prefix == "CAT1":
                # With compact ids we cannot reliably map descendants, so close nested levels.
                self._openedCategories = {k for k in self._openedCategories if not (k.startswith("CAT2") or k.startswith("CAT3"))}
            elif prefix == "CAT2":
                self._openedCategories = {k for k in self._openedCategories if not k.startswith("CAT3")}
            return

        self._openedCategories.add(id)
    # ------------------------------------------------------------------------
    ...
    def addCategoryEntry(
        ...
        if (isVisible):
            ...
            if self._expandAll:
                ...
                catData.isOpen = True
            elif self._autoCollapseCategories and (not self._deferAutoCollapseUntilNextCategory) and (catId == selectedCatId):
                catData.item.prefix = indent + "-"
                catData.isOpen = True
            elif ((not self._autoCollapseCategories) or self._deferAutoCollapseUntilNextCategory) and (("CAT" + str(catPos) + catId) in self._openedCategories):
                catData.item.prefix = indent + "-"
                catData.isOpen = True
            else:
                catData.item.prefix = indent + "+"
    ...
    def getFilteredList(self) -> List[SModUiListItem] :
        ...
        for i in range(0, len(self._items)):
            ...
            if (itemCategories != lastCategories):
                ...
                if self._expandAll:
                    isOpened = True
                elif self._autoCollapseCategories:
                    if self._deferAutoCollapseUntilNextCategory:
                        isOpened = (
                            ("CAT1" + item.cat1 in self._openedCategories)
                            and ((item.cat2 == "") or ("CAT2" + item.cat2 in self._openedCategories))
                            and ((item.cat3 == "") or ("CAT3" + item.cat3 in self._openedCategories))
                        )
                    else:
