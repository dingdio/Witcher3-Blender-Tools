...
from ..importers import import_texarray_create_groups
...
def loadTerrainMaterial(filename):
    _dirpath, file = os.path.split(filename)
    ...
    with open(filename, encoding="utf-8") as file_handle:
        ...
        if ext == ".json":
            ...
            return list(map(TexturePath.from_json, json_data))
        if ext in (".yml", ".yaml"):
            yaml_data = yaml.full_load(data) or {}
            materials = yaml_data.get("world", {}).get("terrain_materials", [])
            return list(map(TexturePath.from_json, materials))
    return None
...
def insert_color(mat, principled, tex, mapping, color_path):
    ...
    tex.image = bpy_image_load_safe(color_path, check_existing=True)
...
def insert_normal_tex(mat, normal_map_node, normal_tex_n, mapping, normal_path, principled):
    ...
    normal_tex_n.image = bpy_image_load_safe(normal_path, check_existing=True)
...
def get_normal_node(principled):
    if principled.inputs["Normal"].is_linked:
        ...
        log.debug("node type: %s", soc.node.type)
        log.debug("socket name: %s", soc.name)
...
def insert_heightmap_to_disp(mat, disp_node, heightmap_texture_node, heightmap_path):
    heightmap_texture_node.image = bpy_image_load_safe(heightmap_path, check_existing=True)
...
def setup_terrain(tearrainMat, heightfield_path=""):
    ...
    if heightfield_path and os.path.isfile(heightfield_path):
        ...
        filename = os.path.basename(heightfield_path)
        tex = get_texture_node(filename, mat)
        if tex == None:
            tex = mat.node_tree.nodes.new("ShaderNodeTexImage")
        insert_heightmap_to_disp(mat, disp_material, tex, heightfield_path)
    ...
    #   color_path: ...texarray.texture_0.tga
    #   normal_path: ...normals.texarray.texture_0.tga
    #   bkgrnd: ...bkgrnd.png
    #   overlay: ...overlay.png
        #COLOUR MAP
        ...
        normal_texture_node.image = bpy_image_load_safe(normal_path, check_existing=True)
        ...
        blend_texture.image = bpy_image_load_safe(item.overlay, check_existing=True)
        ...
        blend_texture_background.image = bpy_image_load_safe(item.bkgrnd, check_existing=True)
...
def start_import(fileName):
    ...
    if not material_path or not os.path.isfile(material_path):
        material_path = get_dev_override("terrain_debug_material", "")
    if not material_path or not os.path.isfile(material_path):
        raise FileNotFoundError("Terrain material file not found. Select a valid .json/.yml file.")

    terrain_mat = loadTerrainMaterial(material_path)
    if not terrain_mat:
        raise ValueError(f"Unsupported or empty terrain material: {material_path}")

    heightfield_path = get_dev_override("terrain_debug_heightfield", "")
    setup_terrain(terrain_mat, heightfield_path=heightfield_path)
