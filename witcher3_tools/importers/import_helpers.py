...
try:
    ...
    from .. import get_fbx_uncook_path, get_uncook_path, get_witcher2_game_path
    ...
    if level.CSectorData:
        ...
        for idx, block in enumerate(level.CSectorData.BlockData):
            ...
            if block.resourceIndex < 12:
                ...
                log.debug("%s %s", block.resourceIndex, this_resource)
...
def _transform_real(transform, name, default=0.0):
    ...
    if isinstance(transform, dict):
        ...
        value = getattr(transform, name, default)
    return _coerce_real(value, default)
...
def set_blender_pose_bone_transform(obj, EngineTransform, rotate_180 = False):
    ...
    if isinstance(EngineTransform, dict) or hasattr(EngineTransform, "Scale_x"):
        obj.scale[0] = _transform_real(EngineTransform, "Scale_x", 1.0)
...
def set_blender_object_transform(obj, EngineTransform, rotate_180 = False, from_this_object= False, pose_bone = None, no_rotation=False):
    ...
    mat.translation = [loc_x, loc_y, loc_z] #obj.location
